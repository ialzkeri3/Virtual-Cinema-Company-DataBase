...
BEGIN
	IF NOT i_sortBy='creditCardCount' THEN
		...
		SELECT user.username, count(*) as creditCardCount, status 
		FROM user LEFT JOIN customercreditcard on user.username=customercreditcard.username LEFT JOIN manager ON user.username=manager.username
