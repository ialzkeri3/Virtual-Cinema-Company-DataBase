...
BEGIN
	IF NOT i_sortBy='creditCardCount' THEN
		...
		SELECT user.username, count(*) as creditCardCount,CONCAT(IF(customer.username IS NOT NULL AND employee.username IS NOT NULL, 'CustomerManager',''),IF(customer.username IS NOT NULL AND employee.username IS NULL AND admin.username IS NULL, 'Customer',''),IF(customer.username IS NOT NULL AND admin.username IS NOT NULL, 'CustomerAdmin',''),IF(user.userType='User', 'User',''),IF(customer.username IS NULL AND employee.username IS NOT NULL, 'Manager',''),IF(customer.username IS NULL AND admin.username IS NOT NULL, 'Admin','')) AS userType, status 
		FROM user LEFT JOIN customercreditcard on user.username=customercreditcard.username LEFT JOIN employee ON user.username=employee.username LEFT JOIN admin ON user.username=admin.username LEFT JOIN customer ON user.username=customer.username		WHERE (i_username='' OR user.username=i_username) AND (i_status='ALL' OR status=i_status)
