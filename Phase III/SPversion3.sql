...
BEGIN	
	...
	IF NOT (i_sortBy='NumCityCover' OR i_sortBy='NumTheater' OR i_sortBy='NumEmployee') THEN    
		...
		SELECT company.comName, COUNT(DISTINCT(thCity)) as NumCityCover, COUNT(thName) as NumTheater, COUNT(manUsername) as NumEmployee
		FROM company LEFT JOIN theater ON company.comName=theater.comName
		GROUP BY company.comName
		HAVING 
			(i_comName='' OR company.comName=i_comName) AND
		...
		ORDER BY (CASE WHEN (i_sortBy='' AND i_sortDirection='') THEN company.comName END) DESC,
				 (CASE WHEN (i_sortBy='' AND i_sortDirection='ASC') THEN company.comName END) ASC,
				 (CASE WHEN (i_sortBy='' AND i_sortDirection='DESC') THEN company.comName END) DESC,
				 (CASE WHEN (i_sortBy='comName' AND i_sortDirection='') THEN company.comName END) DESC,
				 (CASE WHEN (i_sortBy='comName' AND i_sortDirection='ASC') THEN company.comName END) ASC,
				 (CASE WHEN (i_sortBy='comName' AND i_sortDirection='DESC') THEN company.comName END) DESC;
...
BEGIN
    ...
    IF i_includeNotPlayed IS TRUE THEN
		...
		SELECT movName, duration, movReleaseDate, movPlayDate 
		FROM movieplay JOIN theater using (thName, comName) JOIN movie using (movName, movReleaseDate)
		WHERE 
...
BEGIN
	 IF i_movReleaseDate<=i_movPlayDate THEN 
		INSERT INTO customerviewmovie(creditCarNum, thName, comName,movName, movReleaseDate, movPlayDate) VALUES(i_creditCardNum, i_thName, i_comName, i_movName, i_movRealeaseDate, i_movPlayDate);
	ELSE 
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT= 'PlayDate Before ReleaseDate';
	END IF;
END$$
